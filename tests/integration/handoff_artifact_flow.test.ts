...
describe('handoff artifact flow', () => {
  it('produces schema-compliant artifacts and lists them for consumers', async () => {
    const artifactsDir = mkdtempSync(join(tmpdir(), 'handoff-artifacts-'));
    ...
    const record = await service.writeHandoffArtifact({
      ...
      nextAction: 'Run tests and implement models/services.',
