...
export class MetricsService {
  ...
  async buildPortfolioSnapshot(): Promise<MetricsSnapshot[]> {
    ...
    for (const entry of entries) {
      ...
      const workItem = JSON.parse(raw) as WorkItemProps & {
      ...
      logger.debug('Computed metrics entry', { workItemId: workItem.id, leadTimeDays, wip });
    }
  ...
  private computeWip(
    workItem: WorkItemProps & { remainingTasks?: string[]; completedTasks?: string[] },
  ): number {
    if (workItem.status === 'completed') {
      return 0;
    }
    const tasks = workItem.remainingTasks ?? [];
