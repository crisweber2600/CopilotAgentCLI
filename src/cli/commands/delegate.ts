...
export async function delegateCommand(
  ...
  if (interactiveFlag && nonInteractiveFlag) {
    throw new ValidationError(
      "Select either --interactive or --non-interactive, not both.",
    );
  }
  ...
  const files = getStringArrayFlag(parsed, "file").map((file) =>
    resolvePath(context.cwd, file),
  );
  ...
  const branchOverride =
    getStringFlag(parsed, "branch") ?? getStringFlag(parsed, "base-branch");
  const quiet = hasFlag(parsed, "quiet");
  const output = resolveOutputFormat(
    parsed,
    quiet ? true : context.ciDefaultJson,
  );
  ...
  if (
    !repository &&
    context.sessionService.isRemote() &&
    !context.env.COPILOT_CLI_TEST_MODE
  ) {
    throw new ValidationError(
  ...
  if (!repository && options.branch) {
    throw new ValidationError(
      "Cannot override branch when repository information is unavailable.",
    );
  }
  ...
  if (options.output === "json") {
    ...
    writeLine(
      context.stdout,
      `Delegated session ${session.id} (status: ${session.status}).`,
    );
  }
...
function validateInputContext(
  ...
  if (missing.length > 0) {
    throw new ValidationError(
      `Missing context references: ${missing.join(", ")}`,
    );
  }

  const contextValidation = validateContextRefs(request.contextRefs, (path) =>
    existsSync(path),
  );
  if (!contextValidation.valid) {
    const reason =
      "reason" in contextValidation
        ? contextValidation.reason
        : "Context references are invalid.";
    throw new ValidationError(reason);
  }
}
