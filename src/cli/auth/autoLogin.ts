...
function nonInteractive(context: CliContext): boolean {
  return (
    envTruthy(context.env.COPILOT_CLI_NON_INTERACTIVE) ||
    envTruthy(context.env.CI)
  );
}
...
export async function attemptAutoLogin(context: CliContext): Promise<boolean> {
    ...
    try {
      ...
      if (error instanceof AuthError) {
        ...
        context.stderr.write(
          `Failed to authenticate using COPILOT_AGENT_TOKEN: ${error.message}\n`,
        );
      }
      return false;
    }
  ...
  if (nonInteractive(context)) {
    context.stderr.write(
      "Automatic login skipped (non-interactive environment).\n",
    );
    return false;
  }
  ...
  try {
    if (!shouldAttemptBrowserOpen(context)) {
      context.stderr.write(
        "Follow the on-screen instructions to complete sign-in.\n",
      );
    }
