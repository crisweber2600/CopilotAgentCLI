...
export class Assignment {
  ...
  constructor(props: AssignmentProps) {
    ...
    if (!props.executorId) {
      throw new Error(`Assignment.executorId is required for attempt ${props.attemptId}`);
    }
    if (!props.claimedAt) {
      throw new Error(`Assignment.claimedAt is required for attempt ${props.attemptId}`);
    }
