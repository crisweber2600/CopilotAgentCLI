...
1. **Install the CLI** – Publish the tarball created by `npm run package` or install directly via git (`npm install --save-dev git+https://github.com/crisweber2600/CopilotAgentCLI.git`). The package’s `prepare` hook compiles the CLI so `node_modules/.bin/copilot-cli` is ready to use.
2. **Run the bootstrap** – From the destination repository, execute `npx copilot-cli init`. This single command copies the orchestrator/execute/collector workflows, seed `artifacts/`, `specs/`, docs, and the Speckit sync utilities. Use `--force` to overwrite existing files or `--target <dir>` to populate a different directory.
3. **Author specs with Speckit** – Generate `specs/<feature-id>/` outputs, run the bundled sync script (`node scripts/sync-speckit-artifacts.mjs <feature-id>`) inside the target repo, and commit the results. The CLI and workflows consume these files directly.
4. **Delegate work** – Trigger `.github/workflows/orchestrator.yml` or run `copilot-cli delegate --structured-work-item <work-item-id>` locally to produce schedules.
5. **Execute & validate** – Allow `.github/workflows/execute.yml` to fan out attempts, producing handoff artifacts and exercising the baseline guards.
6. **Collect history** – Enable `.github/workflows/collector.yml` to commit `artifacts/` updates, keeping a tamper-evident trail.
