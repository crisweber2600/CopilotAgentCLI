...
2. **Given** steps *B* and *C* are marked **parallelizable** and become ready after *A*, **When** scheduling runs and executors are available, **Then** *B* and *C* are **triggered concurrently** to different executors with **launch order = ascending step index**; both must finish before *D* starts.
...
* **FR‑003**: Provide a **human pre‑handoff review gate** before worker execution to merge/link/close suspected duplicates with recorded rationale. *(No automated duplicate detection required for MVP.)*
